...
public class LargeInteger {
  ...
  public LargeInteger multiply10() {
        // TODO
	    if(head.data == 0 && head.link == null) return this;
  ...
  public LargeInteger add(LargeInteger that) {
        // TODID
	    LargeInteger proxyThis = new LargeInteger(this.toString());
	    LargeInteger proxyThat = new LargeInteger(that.toString());
	    int currDigit;
	    int remainder = 0;
	    String sumString = "";
	    while(!(proxyThis.head.data == 0 && proxyThis.head.link == null) || !(proxyThat.head.data == 0 && proxyThat.head.link == null)) {
	    	currDigit = proxyThis.head.data + proxyThat.head.data + remainder;
	    	remainder = (currDigit > 9) ? 1 : 0;
	    	currDigit %= 10;
	    	sumString = currDigit + sumString;
	    	proxyThis.divide10();
	    	proxyThat.divide10();
	    }
	    if(remainder != 0) sumString = remainder + sumString;
        return new LargeInteger(sumString);
  }
  ...
  public LargeInteger multiply(int x) {
        // TODID
	    LargeInteger toReturn = new LargeInteger("0");
	    for(int i = 0; i < x; i++) toReturn = toReturn.add(this);
        return toReturn;
  }
  ...
  private String toString(LLNode<Integer> node) {
        // TODID
        return node.link == null ? Integer.toString(node.data) : toString(node.link) + node.data;
  }
